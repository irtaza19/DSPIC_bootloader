...
uint32_t getInt4StringMISC(uint8_t * data)
{
    uint8_t * ptr = data;
	uint8_t k = 0;
    uint8_t flag = 1;
    ...
    uint32_t res = 0;
    /* uint16_t nmax = len; */
    uint16_t nmax = len - cnt;

    for(k = 0; k < nmax; k++)
    {
        res = ((uint32_t)(ptr[k]-0x30)) + (res * 10);
    }
    return res;
}

uint32_t getNum4StringMISC(uint8_t * data, uint8_t limiter)
{
	uint8_t k;
	uint8_t flag = 0;
	uint32_t res = 0;
	...
	if(flag)
	{
		...
		for(n=0;n<nmax;n++)
		{
			res += base*((uint32_t)(data[k-1]-0x30));
			k--;
			base = base*10;
		}
	}
...
void printStrMISC(uint8_t * dstr, char *str, ... )
{
